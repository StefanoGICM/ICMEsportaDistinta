/****** Object:  Table [dbo].[XPORT_DIST]    Script Date: 12/1/2022 4:55:37 PM ******/
SET ANSI_NULLS ON
GO

SET QUOTED_IDENTIFIER ON
GO

CREATE TABLE [dbo].[SWBOM](
    [SessionID] [uniqueidentifier] NOT NULL,
	[DEDIDP] [nvarchar](50) NOT NULL,
	[DEDREVP] [nvarchar](50) NOT NULL,
	[DEDIDC] [nvarchar](50) NOT NULL,
	[DEDREVC] [nvarchar](50) NOT NULL,
	[QTA] [decimal](23,11) NOT NULL DEFAULT (0),
	[CONSUMO] [decimal] (23, 11) NOT NULL DEFAULT (0),
	[UMCONSUMO] [nvarchar](50) NOT NULL DEFAULT (''),
    [DateIns] [datetime] NULL,
	[DateUpd] [datetime] NULL,
 CONSTRAINT [PK_SWBOM] PRIMARY KEY CLUSTERED 
(
	[SessionID] ASC,
	[DEDIDP] ASC,
	[DEDREVP] ASC,
	[DEDIDC] ASC,
	[DEDREVC] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO

CREATE INDEX IX_SWBOM_DateUpd ON [dbo].[SWBOM] ([DateUpd]);
GO

CREATE INDEX IX_SWBOM_DateIns ON [dbo].[SWBOM] ([DateIns]);
GO